(function () {
  ...
  function inlineSyntaxToHtml(text) {
    ...
    while (remaining.length > 0) {
      ...
      } else if (next.type === 'bb-code' || next.type === 'md-code') {
        html += `<code>${escapeHtml(next.match[1])}</code>`;
  ...
  function collectCodeBlock(lines, startIndex) {
    ...
    const openMatch = firstLine.match(/^\s*\[code](.*)$/i);
    if (!openMatch) {
      return { nextIndex: startIndex + 1, html: renderParagraph([firstLine]) };
    }

    const contentLines = [];
    let currentLine = openMatch[1] || '';
    let index = startIndex;

    while (index < lines.length) {
      const closeIndex = currentLine.search(/\[\/code]/i);
      if (closeIndex >= 0) {
    ...
    const code = escapeHtml(normalizeCodeBlockText(contentLines.join('\n')));
    return {
  ...
  function renderBlocks(text, short) {
    ...
    while (index < lines.length) {
      ...
      if (/^\[code]/i.test(trimmed)) {
        flushParagraph();
        const renderedCodeBlock = collectCodeBlock(lines, index);
        if (renderedCodeBlock.html) blocks.push(renderedCodeBlock.html);
        index = renderedCodeBlock.nextIndex;
        continue;
      }
  ...
  function serializeCodeBlock(node) {
    ...
    if (tag === 'PRE') {
      const text = normalizeCodeBlockText(node.textContent || '');
      return `[code]\n${text}\n[/code]`;
    }
    ...
    const lines = Array.from(node.children)
      .filter(function (child) {
        return child instanceof Element && child.classList.contains('ql-code-block');
      })
      .map(function (child) {
        return normalizeText(child.textContent || '').replace(/\n/g, '');
      });
  ...
  function serializeBlockNode(node) {
    ...
    if (tag === 'PRE' || node.classList.contains('ql-code-block-container')) {
  ...
  window.OWDescriptionCodec = {
    normalizeText,
    normalizeUrl,
    toHtml: function (text, short) {
