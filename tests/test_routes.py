...
class RouteTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_catalog_scripts_support_modpack_catalog(self) -> None:
        ...
        self.assertIn("const renderEntityKind = isGameMode ? 'game' : (isModpackMode ? 'modpack' : 'mod');", catalog_script)
        ...
        self.assertIn("function resolveCatalogState(params = URLManager.getParams())", catalog_params)
        self.assertIn("function applyCatalogState(state = resolveCatalogState())", catalog_params)
        self.assertIn("function renderCatalogSeparators()", catalog_params)
        ...
        self.assertIn("function resetCatalogAfterRangeFeed(rangeFeedPromise)", catalog_params)
        self.assertIn("resetCatalogAfterRangeFeed(loadCatalogRangeFeed());", catalog_params)
        self.assertIn("data-catalog-views", index)
        self.assertNotIn("window.location.assign(", catalog_params)
        self.assertNotIn("catalog-mode-switch__item", catalog_params)
        self.assertIn("const ownerType = params.get('sgame', 'yes') === 'yes'", catalog_params)
    ...
    def test_catalog_conflict_filter_uses_relation_picker_pattern(self) -> None:
        ...
        self.assertIn('catalog-conflicts-setting catalog-mod-only" data-catalog-views="mod"', index)
        self.assertNotIn("<hr", index)
        ...
        self.assertIn("requestSettings.set('excluded_conflicts', excludedConflicts);", catalog_vendor)
        self.assertIn("requestSettings.pop('excluded_conflicts');", catalog_vendor)
        self.assertIn("requestSettings.set('sgame', 'no');", catalog_vendor)
    ...
    def test_catalog_game_type_filter_maps_to_manager_types_query(self) -> None:
        ...
        self.assertIn('catalog-game-type-setting catalog-game-select-filter" data-catalog-views="game"', index)
        ...
        self.assertIn("const sgame = params.get('sgame', 'yes') === 'yes';", catalog_script)
        self.assertIn("const isGameMode = settings.get('sgame', 'yes') == 'yes';", catalog_vendor)
        ...
        self.assertIn('[data-catalog-views][hidden]', catalog_styles)
        self.assertIn('catalog-generated-separator', catalog_script)
    ...
    def test_catalog_sort_resets_to_supported_field_per_mode(self) -> None:
        ...
        self.assertIn("if (mode === 'modpack')", catalog_script)
        ...
        self.assertIn("const sortMode = catalogKind === 'modpack'", catalog_script)
        self.assertIn("sortState = syncCatalogSortForMode(currentSort, sortMode);", catalog_script)
        ...
        self.assertIn('option class="rating" value="rating">Сортировка по рейтингу</option>', index)
